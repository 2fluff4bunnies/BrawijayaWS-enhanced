...
namespace BrawijayaWorkshop.Win32App.ModulControls
{
    public partial class SpecialSparepartListControl : BaseAppUserControl, ISpecialSparepartListView
    {
        ...
        private void bgwExport_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {
                MethodBase.GetCurrentMethod().Fatal("An error occured while trying to export SpecialSparepart", ex);
                e.Result = ex;
            }
        }
        ...
        private void bgwExport_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            if (e.Result is Exception)
            {
                this.ShowError("Proses export SpecialSparepart gagal!");
            }

            FormHelpers.CurrentMainForm.UpdateStatusInformation("Export SpecialSparepart selesai", true);
        }
        ...
        private void btnExportToCSV_Click(object sender, EventArgs e)
        {
            if (!bgwExport.IsBusy && !bgwMain.IsBusy)
            {
                ...
                btnSearch.PerformClick();
                exportDialog.FileName = "SpecialSparepart_" + DateTime.Now.ToString("yyyyMMdd_HHmmssfff") + ".csv";
        ...
        private void exportDialog_FileOk(object sender, CancelEventArgs e)
        {
            ...
            MethodBase.GetCurrentMethod().Info("Exporting SpecialSparepart data...");
            FormHelpers.CurrentMainForm.UpdateStatusInformation("Proses export data SpecialSparepart...", false);
