...
namespace BrawijayaWorkshop.Win32App.ModulControls
{
    public partial class UsedGoodsTransactionListControl : BaseAppUserControl, IUsedGoodTransactionListView
    {
        ...
        private void bgwExport_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {
                MethodBase.GetCurrentMethod().Fatal("An error occured while trying to export UsedGoodsTransaction", ex);
                e.Result = ex;
            }
        }
        ...
        private void bgwExport_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            if (e.Result is Exception)
            {
                this.ShowError("Proses export UsedGoodsTransaction gagal!");
            }

            FormHelpers.CurrentMainForm.UpdateStatusInformation("Export UsedGoodsTransaction selesai", true);
        }
        ...
        private void btnExportToCSV_Click(object sender, EventArgs e)
        {
            if (!bgwExport.IsBusy && !bgwMain.IsBusy)
            {
                ...
                btnSearch.PerformClick();
                exportDialog.FileName = "UsedGoodsTransaction_" + DateTime.Now.ToString("yyyyMMdd_HHmmssfff") + ".csv";
        ...
        private void exportDialog_FileOk(object sender, CancelEventArgs e)
        {
            ...
            MethodBase.GetCurrentMethod().Info("Exporting UsedGoodsTransaction data...");
            FormHelpers.CurrentMainForm.UpdateStatusInformation("Proses export data UsedGoodsTransaction...", false);
