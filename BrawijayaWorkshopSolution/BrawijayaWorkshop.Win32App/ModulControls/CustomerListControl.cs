...
namespace BrawijayaWorkshop.Win32App.ModulControls
{
    public partial class CustomerListControl : BaseAppUserControl, ICustomerListView
    {
        ...
        private void bgwExport_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {
                MethodBase.GetCurrentMethod().Fatal("An error occured while trying to export Customer", ex);
                e.Result = ex;
            }
        }
        ...
        private void bgwExport_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            if (e.Result is Exception)
            {
                this.ShowError("Proses export Customer gagal!");
            }

            FormHelpers.CurrentMainForm.UpdateStatusInformation("Export Customer selesai", true);
        }
        ...
        private void btnExportToCSV_Click(object sender, EventArgs e)
        {
            if (!bgwExport.IsBusy && !bgwMain.IsBusy)
            {
                ...
                btnSearch.PerformClick();
                exportDialog.FileName = "Customer_" + DateTime.Now.ToString("yyyyMMdd_HHmmssfff") + ".csv";
        ...
        private void exportDialog_FileOk(object sender, CancelEventArgs e)
        {
            ...
            MethodBase.GetCurrentMethod().Info("Exporting Customer data...");
            FormHelpers.CurrentMainForm.UpdateStatusInformation("Proses export data Customer...", false);
            bgwExport.RunWorkerAsync();
        }

        private void btnPrint_Click(object sender, EventArgs e)
        {
            if (!gridCustomer.IsPrintingAvailable)
            {
