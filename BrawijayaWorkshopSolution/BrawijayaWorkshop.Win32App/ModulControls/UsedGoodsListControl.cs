...
namespace BrawijayaWorkshop.Win32App.ModulControls
{
    public partial class UsedGoodsListControl : BaseAppUserControl, IUsedGoodListView
    {
        ...
        private void bgwExport_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {
                MethodBase.GetCurrentMethod().Fatal("An error occured while trying to export UsedGoods", ex);
                e.Result = ex;
            }
        }
        ...
        private void bgwExport_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            if (e.Result is Exception)
            {
                this.ShowError("Proses export UsedGoods gagal!");
            }

            FormHelpers.CurrentMainForm.UpdateStatusInformation("Export UsedGoods selesai", true);
        }
        ...
        private void btnExportToCSV_Click(object sender, EventArgs e)
        {
            if (!bgwExport.IsBusy && !bgwMain.IsBusy)
            {
                ...
                btnSearch.PerformClick();
                exportDialog.FileName = "UsedGoods_" + DateTime.Now.ToString("yyyyMMdd_HHmmssfff") + ".csv";
        ...
        private void exportDialog_FileOk(object sender, CancelEventArgs e)
        {
            ...
            MethodBase.GetCurrentMethod().Info("Exporting UsedGoods data...");
            FormHelpers.CurrentMainForm.UpdateStatusInformation("Proses export data UsedGoods...", false);
