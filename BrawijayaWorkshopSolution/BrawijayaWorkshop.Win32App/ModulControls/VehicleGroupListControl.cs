...
namespace BrawijayaWorkshop.Win32App.ModulControls
{
    public partial class VehicleGroupListControl : BaseAppUserControl, IVehicleGroupListView
    {
        ...
        private void bgwExport_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {
                MethodBase.GetCurrentMethod().Fatal("An error occured while trying to export VehicleGroup", ex);
                e.Result = ex;
            }
        }
        ...
        private void bgwExport_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            if (e.Result is Exception)
            {
                this.ShowError("Proses export VehicleGroup gagal!");
            }

            FormHelpers.CurrentMainForm.UpdateStatusInformation("Export VehicleGroup selesai", true);
        }
        ...
        private void btnExportToCSV_Click(object sender, EventArgs e)
        {
            if (!bgwExport.IsBusy && !bgwMain.IsBusy)
            {
                ...
                btnSearch.PerformClick();
                exportDialog.FileName = "VehicleGroup_" + DateTime.Now.ToString("yyyyMMdd_HHmmssfff") + ".csv";
        ...
        private void exportDialog_FileOk(object sender, CancelEventArgs e)
        {
            ...
            MethodBase.GetCurrentMethod().Info("Exporting VehicleGroup data...");
            FormHelpers.CurrentMainForm.UpdateStatusInformation("Proses export data VehicleGroup...", false);
