    ...
    public partial class ManualTransactionListControl : BaseAppUserControl, IManualTransactionListView
    {
       ...
       #region View's Member
        ...
        private void bgwExport_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {
                MethodBase.GetCurrentMethod().Fatal("An error occured while trying to export ManualTransaction", ex);
                e.Result = ex;
            }
        }
        ...
        private void bgwExport_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            if (e.Result is Exception)
            {
                this.ShowError("Proses export ManualTransaction gagal!");
            }

            FormHelpers.CurrentMainForm.UpdateStatusInformation("Export ManualTransaction selesai", true);
        }
        ...
        private void btnExportToCSV_Click(object sender, EventArgs e)
        {
            if (!bgwExport.IsBusy && !bgwMain.IsBusy)
            {
                ...
                btnSearch.PerformClick();
                exportDialog.FileName = "ManualTransaction_" + DateTime.Now.ToString("yyyyMMdd_HHmmssfff") + ".csv";
        ...
        private void exportDialog_FileOk(object sender, CancelEventArgs e)
        {
            ...
            MethodBase.GetCurrentMethod().Info("Exporting ManualTransaction data...");
            FormHelpers.CurrentMainForm.UpdateStatusInformation("Proses export data ManualTransaction...", false);
